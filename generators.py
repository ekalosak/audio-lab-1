...
import numpy as np
#----------------------------------------------------------------------------------------#
#------------------ All the classes below should have next_note() method ----------------#	
#----------------------------------------------------------------------------------------#	
class Markov:
	...
	def transition_matrix(self):
		...
		song = self.song + self.song[:self.ord]		# augment song so to mimic wrapping
	...
	def sample(self,dist): 					# return samples from categorical distribution
		dist = dist / (np.sum(dist) + .001) # by using sequence of probabilities to define
											# to define series of thresholds in interval.
