...
class BoomBox:
	...
	def get_next_sound(self):
		sound_bytes = [v.next_sound() for v in self.voices]
		sound_bytes = [sound for sound in sound_bytes if sound != None]
		output = np.zeros(self.dt)
		for sound in sound_bytes: output += sound / len(sound_bytes)
		return ''.join(struct.pack('h', int(samp)) for samp in output)
#----------------------------------------------------------------------------------------#		
class Voice:
	...
	def new_sound(self):
		n = self.note_gen.next_note()
		s = [self.sound_ref[x] for x in n[0]]
		a = np.zeros(self.parent.tmeas * fs / n[1])
